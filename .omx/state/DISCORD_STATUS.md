...
- 현재 목표: Discord ingest/relay + OMX 회의 루프를 최신 지시 기준으로 안정화하고, 로컬 스모크와 verify gate까지 같은 경로로 검증한다.
...
- quick smoke: `python scripts/test_discord_bridge.py`
- verify gate: `python scripts/no_secrets_guard.py` -> `python scripts/verify_minimal.py`
...
- `pnpm verify:automation` 이 로컬에서 통과한다.
