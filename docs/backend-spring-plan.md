...
- 기존 FastAPI 앱은 프런트 저장소에서 제거한다. Python이 필요하면 API 서버가 아니라 데이터 수집·LLM 요약·Perso 더빙 worker로 별도 분리한다.
...
- 문서: `/v3/api-docs`, `/swagger-ui.html`
- 인증 전략: 백엔드 `HttpOnly` access/refresh cookie, refresh token hash 저장, refresh 시 token 회전
- CI: `verify`, `dependency-review`, `codeql`
- GitHub 설정: squash merge 비활성화, branch 자동 삭제 비활성화, `main`/`develop` 보호 적용

## 남은 연동 작업

1. 프런트의 `STOCK_API_BASE_URL`을 Spring Boot API로 고정하고 Docker/검증 경로를 정리한다.
2. Spring Boot 화면 응답 계약과 `packages/contracts` 타입 차이를 계속 줄인다.
