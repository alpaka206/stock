...
### What / Why / How
- TBD 4TBD IA? TBD? ? `/news`, `/calendar` TBD route? TBD API/TBD? TBDTBD.
- Alpha Vantage `EARNINGS_CALENDAR` / `IPO_CALENDAR` CSV TBD? TBDTBD? client? TBDTBD.
- OpenDART TBD TBD TBD/TBD TBD? `news`, `calendar`? TBDTBD.
- provider env TBD, env example, README, manifest, ADR, TBD/TBD?/TBD TBD? TBD TBDTBD.

### ?
- TBDTBD TBD TBDTBD TBD? TBD? TBDTBD TBD? TBD? ? TBD? TBD.
- TBD? TBD? TBD sidebar 4TBD? TBDTBD TBD TBD? TBD route TBD? TBDTBD.
- real `/calendar` TBD? CSV helper TBD TBD? TBD?, unattended TBD? TBD TBD TBD? TBD? ? TBD TBD? TBDTBD TBD.

### What / Why / How
- backend? `extended_real` / `extended_mock` provider? `news`, `calendar` router / schema / contract / prompt? TBDTBD.
- web? `/news`, `/calendar` route? loader, fixture, topbar route context, overview CTA? TBDTBD.
- `scripts/api_smoke.py`? schema validationTBD TBDTBD? TBDTBD `pnpm verify:standard`? TBD TBD? TBD TBDTBD?.
- page manifest, component manifest, design memory, ADR-0008, env/deployment/test docs, OMX plan/log, issue/PR draft? TBD TBD?.
...
### What / Why / How
- TBD 4TBD TBD TBD? Gemini TBD provider? deterministic fallback? TBDTBD.
- `RESEARCH_LLM_PROVIDER`, `GEMINI_API_KEY`, `GEMINI_MODEL`, `GEMINI_BASE_URL` env? TBDTBD.
- readiness? OpenAI TBD TBD? TBD? OpenAI/Gemini/none TBD? TBDTBD? TBDTBD.

### ?
- `OPENAI_API_KEY`? TBD ? TBD 4TBD real path? TBD TBD? TBD? TBD? TBD.
- TBD TBD TBD? TBD LLM TBD? TBD? TBD?/TBD TBD TBD? TBDTBD.

### What / Why / How
- `ResearchSummaryClient`? OpenAI -> Gemini -> deterministic fallback TBD? TBD provider? TBDTBD TBD.
- Gemini REST structured output client? TBDTBD, TBD ? source-based deterministic summary? TBDTBD TBDTBD.
- env example? README/docs? TBD TBDTBD.
