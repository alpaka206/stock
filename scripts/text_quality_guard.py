...
DOC_PLACEHOLDER_EXTS = {".md", ".txt", ".jsonl"}
PLACEHOLDER_EXTS = {".env", ".example", ".py", ".md", ".txt", ".sh", ".ps1", ".toml", ".yaml", ".yml", ".json", ".jsonl"}
RAW_UNICODE_EXTS = {".env", ".example", ".md", ".txt", ".toml", ".yaml", ".yml", ".json", ".jsonl"}
...
def main() -> int:
    ...
    for path in iter_candidate_paths():
        ...
        if suffix in PLACEHOLDER_EXTS and PLACEHOLDER_RE.search(text):
            failures.append(f"{rel}: placeholder sequence detected")
        if suffix in DOC_PLACEHOLDER_EXTS and DOC_DOUBLE_QUESTION_RE.search(text):
            failures.append(f"{rel}: document placeholder detected")
        if suffix in RAW_UNICODE_EXTS and UNICODE_ESCAPE_RE.search(text):
