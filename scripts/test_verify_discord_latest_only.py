...
def role_entry(meeting_id: str, phase: str) -> dict:
    return {
        "source": "agent",
        "role": phase,
        "phase": phase,
...
def test_snapshot_and_verify_pass() -> None:
    with tempfile.TemporaryDirectory(prefix="latest-only-pass-") as temp_dir:
        ...
        meeting_entries = [role_entry("meeting-1", phase) for phase in ("planner", "critic", "researcher", "architect", "executor", "verifier")]
...
def test_verify_fails_when_executor_metadata_missing() -> None:
    with tempfile.TemporaryDirectory(prefix="latest-only-fail-") as temp_dir:
        ...
        entries = [role_entry("meeting-2", phase) for phase in ("planner", "critic", "researcher", "architect", "executor", "verifier")]
